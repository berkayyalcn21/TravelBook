...
class ViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
            ...
            CLGeocoder().reverseGeocodeLocation(requestLocation) { (placemarks, error) in
                ...
                if let placemark = placemarks{
                    if placemark.count > 0{
                        ...
                        let launchOpsion = [MKLaunchOptionsDirectionsModeKey: MKLaunchOptionsDirectionsModeDriving]
                        item.openInMaps(launchOptions: launchOpsion)
                    }
                }
    ...
    @IBAction func saveButtonCliked(_ sender: Any) {
        ...
        if nameText.text != "" && commentText.text != "" {
            ...
            do{
                try context.save()
                let alert = UIAlertController(title: "Info", message: "Location saved", preferredStyle: UIAlertController.Style.alert)
                let button = UIAlertAction(title: "OK", style: UIAlertAction.Style.default, handler: nil)
                alert.addAction(button)
                self.present(alert, animated: true)
            ...
            let alert = UIAlertController(title: "Error", message: "Don't leave empty space", preferredStyle: UIAlertController.Style.alert)
            let button = UIAlertAction(title: "OK", style: UIAlertAction.Style.default, handler: nil)
            alert.addAction(button)
            self.present(alert, animated: true)
        }
